...
export async function create(post: newPost): Promise<Posts> {
...
export async function findeAllPosts(filter: Record<string, unknown>, fields?: string[], limit = 20, skip = 0): Promise<Posts[] | []> {
    let select = selectFields(fields);
...
export async function updateUserPost(filter: Record<string, unknown>, post: PostInterface): Promise<UpdateResult> {
...
export async function searchPosts(query: string, fields?: string[], limit = 20, skip = 0): Promise<unknown> {
    let select = selectFields(fields);
    return PostRepository.find({
